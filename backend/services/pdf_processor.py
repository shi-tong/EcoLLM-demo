...
# from .lci_extractor import LCIInformationExtractor  # 已移除
...
class EnhancedPDFProcessor:
    ...
    def __init__(self, chunk_size: int = 600, chunk_overlap: int = 150):
        ...
        # 🔥 集成专家建议的表格感知分块器
        self.table_aware_chunker = TableAwareChunker(
            chunk_size=chunk_size,
            chunk_overlap=chunk_overlap,
            table_serialization_format="markdown"  # 使用专家建议的table_section格式
        )
        ...
        self.text_splitter = RecursiveCharacterTextSplitter(
            ...
            keep_separator=True  # 保留分隔符以维护上下文
        )
        # self.lci_extractor = LCIInformationExtractor(use_nlp=False)  # 已移除
    ...
    def process_pdf(self, file_path: str) -> List[Document]:
        ...
        try:
            # 🔥 优先使用表格感知分块器
            logger.info("使用专家建议的表格感知分块策略")
            return self.table_aware_chunker.process_pdf_with_table_awareness(file_path)
                
        except Exception as e:
            logger.error(f"表格感知PDF处理失败: {str(e)}")
            logger.info("降级到传统PDF处理")
            # 如果表格感知处理失败，降级到传统处理
            try:
                ...
                logger.error(f"传统PDF处理也失败: {str(e2)}")
                raise e
    
    def _process_pdf_basic(self, file_path: str, session_id: str = None) -> List[Document]:
        """基础PDF处理（使用PyPDFLoader）"""
        loader = PyPDFLoader(file_path)
        pages = loader.load()
        documents = self.text_splitter.split_documents(pages)
        
        # 为每个文档块分配稳定的全局chunk ID
    ...
    def _process_pdf_enhanced(self, file_path: str, session_id: str = None) -> List[Document]:
        """增强PDF处理（使用pdfplumber + PyPDFLoader）"""
        # 1. 获取基础文档（不分配chunk ID）
        loader = PyPDFLoader(file_path)
        pages = loader.load()
        base_documents = self.text_splitter.split_documents(pages)
        ...
        # 4. 为增强后的文档分配稳定的chunk ID
        enhanced_documents = self._assign_stable_chunk_ids(enhanced_documents, file_path, session_id)
    ...
    def _assign_stable_chunk_ids(self, documents: List[Document], file_path: str, session_id: str = None) -> List[Document]:
        ...
        logger.info(f"为{len(documents)}个文档块分配了稳定的chunk ID")
        return documents
...
class PDFProcessor(EnhancedPDFProcessor):
    ...
    def process_pdf_and_get_full_text(self, file_path: str, session_id: str = None) -> tuple[str, List[Document]]:
        ...
        """
        try:
            # 使用增强处理方法，传递session_id
            if session_id:
                documents = self._process_pdf_enhanced(file_path, session_id)
            else:
            ...
            logger.info(f"PDF处理完成: {len(documents)}个片段, 总文本长度: {len(full_text)}")
